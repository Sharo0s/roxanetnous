...
import { updateAnnonceBeneficiaire } from '@/app/actions/annonces'
...
type Annonce = {
  ...
  message_auxiliaires: string | null
}

export function ModifierAnnonceBeneficiaireForm({ annonce }: { annonce: Annonce }) {
  ...
  const [messageAuxiliaires, setMessageAuxiliaires] = useState(annonce.message_auxiliaires || '')
  ...
  async function handleSubmit() {
    ...
    setLoading(true)
    const result = await updateAnnonceBeneficiaire(annonce.id, {
      titre,
      description,
      ...
      message_auxiliaires: messageAuxiliaires,
    })
      ...
      <div className="space-y-6">
        ...
        <div className="bg-white rounded-xl border p-6">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-2">Diplome souhaite</label>
              <select
                value={diplomeRequis}
            ...
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-2">Experience minimum</label>
              <select
                value={experienceMin}
        ...
        <div className="bg-white rounded-xl border p-6">
          ...
          <div className="space-y-4">
            ...
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-2">
                Message aux auxiliaires
              </label>
              <textarea
                value={messageAuxiliaires}
                onChange={(e) => setMessageAuxiliaires(e.target.value)}
                rows={3}
                className="w-full rounded-lg border border-gray-300 bg-white px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-black resize-none"
              />
            </div>
        ...
        <div className="flex gap-3">
          <Link href="/beneficiaire/annonces">
