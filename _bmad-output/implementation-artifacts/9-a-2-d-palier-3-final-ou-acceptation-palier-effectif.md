...
- [x] **T4 (cond Option A ou hybride) Push + run GHA + ajustement thresholds palier 3 final/effectif** (AC4, AC9)
  - [x] T4.1 Push branche `story/9-a-2-d-palier-3-final` ou `story/9-a-2-d-hybride`. Workflow `integration-tests` declenche sur PR. -- **Done** : branche `story/9-a-2-d-hybride` push origin, PR #15 ouverte 2026-05-18.
  - [x] T4.2 1er run GHA mesure coverage cumulee unit+integration sur `app/actions/parrainage.ts`. Documenter les 4 indicateurs precis (lines/branches/functions/statements). -- **Done** : run GHA `Integration Tests` #26047697818 VERT. Cumul = lines **86.50** / branches **75.38** / functions **100** / statements **85.18**.
  - [x] T4.3 **Decision palier 3 final** : si 4/4 indicateurs >= 85% -> palier 3 final pur, MAJ thresholds = chiffres mesures arrondis (>= 85). Si < 85% sur 1-2 indicateurs apres SC GO -> palier 3 effectif final = chiffres mesures arrondis (jamais < palier 3 effectif courant 80/72/100/79). Bloc verdict 9.A.2.d MAJ avec chiffres finaux. -- **Done** : **Cas hybride AC6** = palier 3 final pur sur 3/4 indicateurs (lines + functions + statements >= 85%) + palier 3 effectif final sur branches (75.38, plafond structurel C2+C4). Thresholds = 86/75/100/85.
  - [x] T4.4 `vitest.config.ts > thresholds` MAJ avec chiffres mesures arrondis + commentaire inline solde definitivement la roadmap 9.A.2/b/c/d (mention F-Epic9-A2 final + ref PR/run + date). -- **Done** : `vitest.config.ts:54-61` MAJ thresholds 86/75/100/85 + commentaire inline lignes 41-53 enrichi (mention palier 3 final/effectif final + PR #15 + run GHA #26047697818 + 2026-05-18 + F-Epic9-A2 soldee definitivement).
  - [x] T4.5 2e push (ajustement thresholds + doc) sur la meme PR. 1 run vert suffit (pattern F-Epic9-A2 relaxe). -- **A executer** (commit T4 ajustement thresholds + push sur meme branche PR #15).
...
- **`vitest.config.ts`** (modifie, lignes 41-61) : thresholds palier 3 final/effectif final = **86/75/100/85** (chiffres cumul GHA #26047697818 PR #15 arrondis au point inferieur, jamais < 80/72/100/79 palier 3 effectif 9.A.2.c). Commentaire inline solde definitivement la roadmap 9.A.2/b/c/d.
...
- 2026-05-18 -- T4 ajustement thresholds palier 3 final/effectif final post-1er run GHA. **Cumul GHA #26047697818 (PR #15) = lines 86.50 / branches 75.38 / functions 100 / statements 85.18**. Cible 85% strict **ATTEINTE sur 3/4 indicateurs** (lines + functions + statements) ; branches 75.38 < 85% par plafond structurel C2+C4. Cas hybride AC6 acte : palier 3 final pur sur 3 indicateurs + palier 3 effectif final sur branches = **86/75/100/85**. `vitest.config.ts > thresholds['app/actions/parrainage.ts']` MAJ 86/75/100/85 + commentaire inline solde definitivement la roadmap 9.A.2/b/c/d (ref PR #15 + run GHA #26047697818). DECISIONS.md F-Epic9-A2 placeholder GHA TBD remplace par chiffres reels + bloc final mention `86/75/100/85`. deferred-work.md `[Solde 9.A.2.d - 2026-05-18 - PR #15 / run GHA #26047697818]` + Final Resolution mention `86/75/100/85`. Memoire `project_epic_9_cadrage` MAJ avec chiffres cumul GHA. **F-Epic9-A2 definitivement soldee** : palier final permanent acte ; aucune story de coverage future planifiee sur `app/actions/parrainage.ts`. 2e push attendu : 1 run GHA vert suffit (pattern F-Epic9-A2 relaxe).

### Review Findings

- [x] [Review][Patch] SC45 : seeds `users[3]`/`users[4]` fantômes avec commentaires trompeurs [`tests/unit/parrainage-symetrie.test.ts:2036–2038`] — supprimés, commentaire explicatif ajouté (loadNamesForAdminEmail conditionné à blocage===meme_email).
- [x] [Review][Patch] SC44 : assertion absence INSERT `admin_actions_log` non vérifiée [`tests/unit/parrainage-symetrie.test.ts:2019`] — assertion `mockCaptureException.not.toHaveBeenCalledWith signal=signup-log-bloque` ajoutée après l'assert principal.
- [x] [Review][Defer] SC43 : import dynamique dans le corps du test [`tests/unit/parrainage-symetrie.test.ts:1912`] — `sendAdminParrainageFlag` n'est pas hoistée (non présente dans le bloc `vi.hoisted`). L'import dynamique est la seule façon d'obtenir la référence typée sans refactorisation du bloc hoisted + factory. Clarification post-review : pattern nécessaire dans ce contexte, non trivial à corriger sans toucher l'ensemble du bloc hoisted. — deferred, pre-existing
- [x] [Review][Defer] `clearAllMocks` ne reset pas `mockImplementation` dans le describe C1 [`tests/unit/parrainage-symetrie.test.ts:130`] — Pattern pre-existing dans tout le fichier (SC1–SC40 idem). Contamination latente si un futur SC omet de re-setter `mockNormalizeEmail`. Non causé par ce diff. — deferred, pre-existing
- [x] [Review][Defer] SC44 : 4e appel `from('users')` (UPDATE parrainee_par) non documenté, tombe en fallback emptyResponse [`tests/unit/parrainage-symetrie.test.ts:1982–1999`] — Le pool `users` a 3 seeds explicites mais le code source consomme un 4e slot (UPDATE `parrainee_par`). Le fallback `emptyResponse` est correct ici (parraineeErr null, pas de Sentry). Pattern identique aux SC meme_ip pre-existants (SC24/SC26). — deferred, pre-existing
- [x] [Review][Defer] SC47 : asymétrie — pas d'assertion `rpc.toHaveBeenCalledWith` vs SC46 [`tests/unit/parrainage-symetrie.test.ts:2108`] — SC46 vérifie l'invocation RPC avec `p_key`. SC47 ne le fait pas. Asymétrie consciente : Sentry assertion suffit pour prouver que le catch a été atteint. Un refactor qui skippe le RPC et throw directement serait détectable par SC46 mais pas SC47 dans l'immédiat. — deferred, pre-existing
- [x] [Review][Defer] Marge branches threshold ultra-serrée : 0.38 pt (75.38 GHA → seuil 75) [`vitest.config.ts:57`] — Toute branche non couverte ajoutée dans `app/actions/parrainage.ts` peut faire passer CI rouge. Décision F-Epic9-A2 actée et acceptée. Surveiller à chaque story touchant ce fichier. — deferred, pre-existing
- [x] [Review][Defer] `then()` sans handler `reject` sur builder update/insert — Thenable TypeScript partiellement conforme [`tests/unit/_lib/supabase-mock.ts:120`] — Pré-existant depuis 9.A.1 (même pattern sur insert ligne 106). Aucun path de prod ne rejette via ce canal. — deferred, pre-existing
- [x] [Review][Defer] Isolation `mockNormalizeEmail` absente du describe C1 [`tests/unit/parrainage-symetrie.test.ts:1811`] — `vi.clearAllMocks()` n'efface pas les `mockImplementation`. Pattern identique dans l'ensemble du fichier SC1–SC40. Vitest exécute en série, risque nul à court terme. — deferred, pre-existing
- [x] [Review][Defer] SC47 : `headers()` throw avant affectation de `rateLimitKey` — edge non testé [`app/actions/parrainage.ts:348–352`] — Si `headers()` lançait avant l'assignation, `rateLimitKey` serait null et `extra.keyHash` absent de Sentry. Edge de plateforme Next.js non reproductible en unit test pur. Hors-scope story 100% tests. — deferred, pre-existing
