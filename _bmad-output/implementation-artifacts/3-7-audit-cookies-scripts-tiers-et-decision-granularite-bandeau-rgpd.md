...
Code review adversarial 2026-05-07 (3 layers : Blind Hunter, Edge Case Hunter, Acceptance Auditor). 8 decision-needed, 8 patches, 5 deferred, 6 dismissed.

#### Decision-needed

- [x] [Review][Decision] Base légale invoquée : art. 6.1.b RGPD vs art. 82 LIL/Directive ePrivacy — la décision et la politique invoquent l'art. 6.1.b pour exempter du consentement, alors que l'exemption cookies relève de l'art. 82 LIL (transposition Directive ePrivacy 2002/58) et de la Délibération CNIL 2020-091 (catégorie « strictement nécessaire »). Erreur juridique fréquente. Choix : (a) reformuler en citant art. 82 LIL + Délibération 2020-091 pour l'exemption cookies, en gardant art. 6.1.b uniquement pour le **traitement** Supabase auth ; (b) conserver la formulation actuelle car l'argumentaire factuel passe.
- [x] [Review][Decision] Affirmation publique `(httpOnly, secure, sameSite=lax)` non vérifiée live — la politique publie ces attributs comme un fait avéré, mais DECISIONS.md elle-même reconnaît "re-audit DevTools live obligatoire avant ouverture département Bretagne". Si le re-audit révèle un défaut (ex: `secure` absent en cas de mauvaise config HTTPS, `sameSite` non défini), l'affirmation publiée devient trompeuse. Choix : (a) retirer ces attributs de la politique en attendant re-audit ; (b) ajouter une réserve "selon la configuration par défaut de @supabase/ssr 0.10.x" ; (c) garder en l'état (basé sur défauts librairie, considéré stable). [`app/politique-de-confidentialite/page.tsx:91`]
- [x] [Review][Decision] "techniquement, ce n'est pas un cookie" pour localStorage — formulation juridiquement contestable car la CNIL (Délibération 2020-091) assimile localStorage aux **traceurs** soumis au même régime que les cookies HTTP. La préférence du bandeau est probablement exemptée (strictement nécessaire à l'ergonomie), mais l'affirmation générale "techniquement pas un cookie" peut être opposée par un contrôle. Choix : (a) reformuler en "stockage local du navigateur, assimilé à un traceur par la CNIL mais exempté de consentement car strictement nécessaire à l'ergonomie du bandeau" ; (b) conserver telle quelle (pédagogiquement claire pour l'utilisateur). [`app/politique-de-confidentialite/page.tsx:97-100` + `DECISIONS.md` entrée 2026-05-07]
- [x] [Review][Decision] Pas de bumping de la clé `cookies-accepted` malgré la modification du libellé — utilisateurs ayant cliqué "Compris" avant 2026-05-07 conservent `cookies-accepted='true'` et **ne verront jamais** le nouveau libellé corrigé (qui rétablit la précision factuelle localStorage vs cookie HTTP). Choix : (a) bumper en `cookies-accepted-v2` pour forcer réaffichage ; (b) ne rien faire (correction mineure, base utilisateur réduite pré-go-live) ; (c) documenter explicitement la décision dans DECISIONS.md. [`components/cookie-banner.tsx:9`]
- [x] [Review][Decision] "Aucun autre cookie HTTP applicatif" — Supabase peut poser plusieurs cookies (chunking `sb-*-auth-token.0`/`.1` quand le token dépasse 4096 octets, et parfois `sb-*-auth-token-code-verifier` lors de flow OAuth/PKCE). DECISIONS.md cite un cookie unique. Choix : (a) confirmer par re-audit DevTools live et corriger DECISIONS.md/politique si plusieurs cookies sont posés ; (b) reformuler en "famille de cookies `sb-<project-ref>-auth-token*`" pour anticiper ; (c) garder en l'état (singulier dominant). [`DECISIONS.md` entrée 2026-05-07 + `app/politique-de-confidentialite/page.tsx:88-93`]
- [x] [Review][Decision] Qualification "sous-traitant art. 28" pour cookies Stripe — Stripe Payments Europe peut agir en **responsable autonome** ou **conjoint** pour ses finalités propres (anti-fraude Radar, conformité KYC), pas uniquement en sous-traitant art. 28. Affirmer cette qualification publiquement dans DECISIONS.md sans nuance peut être juridiquement risqué. Choix : (a) reformuler en "Stripe Payments Europe Ltd. agit en sous-traitant pour le paiement et en responsable autonome pour ses finalités propres (anti-fraude, conformité)" ; (b) retirer la qualification art. 28 et renvoyer simplement aux mentions légales ; (c) garder en l'état. [`DECISIONS.md` entrée 2026-05-07]
- [x] [Review][Decision] Bandeau ~3x plus long → risque débordement mobile — le `<p>` passe de ~25 mots à ~70 mots dans `cookie-banner.tsx`, sans `max-h` ni overflow. Sur viewport <375px, le bandeau peut occuper 40-50% de la hauteur viewport et masquer le contenu sous-jacent. Vérification visuelle DevTools mobile non documentée dans la story. Choix : (a) raccourcir le libellé bandeau (déléguer le détail à la politique de confidentialité, ne garder qu'une phrase + lien) ; (b) ajouter `max-h-[40vh] overflow-y-auto` sur le wrapper ; (c) garder en l'état après vérif visuelle. [`components/cookie-banner.tsx:23-38`]
- [x] [Review][Decision] Bouton "Compris" + clé `cookies-accepted` : sémantique d'acceptation alors qu'aucun consentement n'est requis — faux ami juridique CNIL. Choix : (a) renommer la clé en `cookie-banner-dismissed` et le label en "Fermer" / "J'ai compris" pour clarifier qu'il s'agit d'une fermeture d'information, pas d'un consentement ; (b) garder en l'état (le `<p>` corrigé clarifie qu'il n'y a rien à accepter). [`components/cookie-banner.tsx:14-18, 35-37`]

#### Patches

- [x] [Review][Patch] Politique mentionne "création/modification profil/annonce" mais omet **onboarding initial auxiliaire** — `step-localisation.tsx:65` charge MapRadius pendant l'onboarding initial. Reformuler pour inclure l'onboarding. [`app/politique-de-confidentialite/page.tsx:123-124`]
- [x] [Review][Patch] Politique sur-promet "modification d'une annonce" alors que `modifier-annonce-form.tsx` côté bénéficiaire **n'utilise pas** MapRadius. Aligner sur réalité (création annonce uniquement). [`app/politique-de-confidentialite/page.tsx:123-124`]
- [x] [Review][Patch] `tile.openstreetmap.org` simplifié — réalité : `{a,b,c}.tile.openstreetmap.org`. Préciser "(et sous-domaines a/b/c)" ou utiliser `*.tile.openstreetmap.org`. [`app/politique-de-confidentialite/page.tsx:127`]
- [x] [Review][Patch] `<em> localStorage</em>` : espace de tête dans la balise → italique d'espace, anti-pattern typographique. Déplacer l'espace hors du tag : `dans le <em>localStorage</em>`. [`app/politique-de-confidentialite/page.tsx:97-98`]
- [x] [Review][Patch] `<ul>` à un seul `<li>` après réécriture — sémantique a11y dégradée (lecteurs d'écran annoncent "liste de 1 élément"). Soit garder une liste à 2 items (cookie HTTP + stockage local groupés), soit transformer en `<p>`. [`app/politique-de-confidentialite/page.tsx:88-94`]
- [x] [Review][Patch] `<code>sb-&lt;project-ref&gt;-auth-token</code>` se lit "inférieur project-ref supérieur" en lecteur d'écran — peu accessible. Reformuler en `sb-[project-ref]-auth-token` ou ajouter `aria-label`. [`app/politique-de-confidentialite/page.tsx:90`]
- [x] [Review][Patch] "OSM Foundation, qui ne réalise pas de traçage commercial" — affirmation sur le comportement d'un tiers que le projet ne peut garantir. Reformuler en factuel : "OpenStreetMap Foundation, fondation à but non lucratif" (fait juridique vérifiable, pas une affirmation sur leur comportement futur). [`app/politique-de-confidentialite/page.tsx:127-129`]
- [x] [Review][Patch] DECISIONS.md invoque "acces uniquement post-authentification + acceptation CGU" comme renforcement OSM — l'argument "acceptation CGU" n'est pas une base légale RGPD valide et n'est pas vérifié dans le code (pas de checkbox CGU explicite documentée à l'inscription). Retirer la mention "acceptation CGU" : la base légale art. 6.1.b suffit (ou art. 82 LIL exemption "strictement nécessaire" si patch decision-needed #1 retenu). [`DECISIONS.md` entrée 2026-05-07]

#### Deferred (reportés)

- [x] [Review][Defer] Citation nominative "(Google Analytics, Meta Pixel, Vercel Analytics, etc.)" peut vieillir mal [`app/politique-de-confidentialite/page.tsx:109-111`] — deferred, robustesse rédactionnelle Epic 4.
- [x] [Review][Defer] Règle DECISIONS.md sans mécanisme automatisé d'enforcement (pas de hook CI / test e2e Playwright cookies) — deferred, déjà acté Epic 4 hardening (cf. rapport audit Section 6).
- [x] [Review][Defer] Conclusion conformité publiée avant re-audit DevTools live — deferred, dépendance déjà inscrite à la Définition de Done Epic 3 (re-audit obligatoire avant ouverture département Bretagne).
- [x] [Review][Defer] Politique de confidentialité ne qualifie pas formellement les "sous-traitants" art. 28 RGPD (Vercel/Supabase/Stripe traités comme "prestataires") [`app/politique-de-confidentialite/page.tsx:117-121`] — deferred, lié aux placeholders mentions-légales déjà reportés Epic 4.
- [x] [Review][Defer] Pas de circuit de revalidation continue de la politique (un dev pourrait ajouter `@vercel/analytics` sans rouvrir le rapport) — deferred, doublon avec la deferred précédente sur enforcement automatisé.

#### Dismissed (bruit / faux positifs)

- Borne AC8 ≤5 fichiers dépassée si on compte sprint-status.yaml + story file — convention BMad : artefacts de gouvernance hors borne.
- Titre DECISIONS.md sans tiret cadratin — convention projet DECISIONS.md sans accents/cadratins, cohérent avec décisions précédentes.
- Date "mai 2026" moins précise que "2026-05-07" — convention projet pages publiques : mois+année (cohérent CGU/mentions-légales).
- Duplication exclusions cookies/scripts dans la politique — redondance pédagogique volontaire.
- Affirmations "0 occurrence" non auditables depuis le diff — Blind Hunter sans contexte ; rapport audit fournit la preuve.
- "5 call-sites" précis dans DECISIONS.md vs formulation publique générique — DECISIONS.md = audit interne, politique = vulgarisée. Normal.
- Numérotation hybride "Contexte 2 bis" dans le rapport audit — lisibilité acceptable pour audit interne.
