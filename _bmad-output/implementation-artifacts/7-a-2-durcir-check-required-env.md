...
Code review post-merge 2026-05-14 sur commits `1894bb5..b935693` (couvre stories 7.A.1 + 7.A.2). Findings specifiques a 7.A.2 ci-dessous.

- [ ] [Review][Decision] `SUPABASE_URL` regex `/^https:\/\/[a-z0-9-]+\.supabase\.(co|com)\/?$/` rejette majuscules + self-hosted [scripts/check-required-env.mjs:29] — si Supabase change project ID rules ou si pivot self-hosted, build prod casse. Decision : (a) elargir a `[a-zA-Z0-9-]+`, (b) garder strict + ajouter doc "Cloud only, override script pour self-hosted", (c) accepter le risque.
- [x] [Review][Patch] `truncate(value)` expose 20 chars du secret dans logs build Vercel [scripts/check-required-env.mjs:101,114] — si placeholder regex faux-positive sur secret reel, 20 chars (~ 80 bits) en logs build. Reduire a 8 chars + ellipsis OU afficher uniquement le prefixe lexical detecte (`'your_'`).
- [x] [Review][Patch] `PLACEHOLDER_RE = /^(your_|xxx|changeme|placeholder|exemple|example)/i` faux-positif sur `Example*` insensible casse [scripts/check-required-env.mjs:92] — `RESEND_FROM_EMAIL="Example Company <noreply@example.com>"`, `SENTRY_ORG=example-corp` bloques. Forcer underscore apres : `/^(your_|xxx_|changeme_|placeholder_|example_|exemple_)/i`.
- [x] [Review][Patch] `STRIPE_WHSEC` regex `/^whsec_[A-Za-z0-9]+$/` trop strict [scripts/check-required-env.mjs:24] — n'accepte ni `_` ni `-`. Si Stripe ajoute prefixe interne (`whsec_test_xxx`, `whsec_snap_xxx`) post-rotation, build casse. Elargir a `/^whsec_[A-Za-z0-9_]+$/`.
- [x] [Review][Patch] `RESEND_KEY` regex `/^re_[A-Za-z0-9_]+$/` rejette `-` [scripts/check-required-env.mjs:25] — Resend pourrait rotate format. Elargir a `/^re_[A-Za-z0-9_-]+$/` (superset urlsafe-base64, cout negligeable).
- [x] [Review][Patch] Test `(j)` couple a un ordre placeholder-first non documente [tests/unit/check-required-env.test.ts:163-171 + scripts/check-required-env.mjs:111-117] — `your_supabase_anon_key` (22 chars) viole AUSSI minLength(40). Selon ordre du code, message differe. Documenter l'ordre placeholder-first en commentaire au-dessus de `validatePresent`.
- [x] [Review][Patch] Pas de tests sur valeurs limites de longueur [tests/unit/check-required-env.test.ts] — manque cas `CRON_SECRET='a'.repeat(31)` (juste sous min) et `'a'.repeat(32)` (juste au min). Off-by-one dans `minLength` (`>` vs `>=`) non detectable. Ajouter 3 cas (31/32 CRON_SECRET, 19/20 SENTRY_AUTH_TOKEN, 39/40 SUPABASE_SERVICE_ROLE_KEY).
- [x] [Review][Patch] `validatePresent` parcourt `allTracked` sans dedup REQUIRED ↔ OPTIONAL_ON_PREVIEW [scripts/check-required-env.mjs:139,144,165] — si refactor declare une var dans les 2 listes, validation 2x + 2 messages. Ajouter assertion top-level : `if (new Set(allTracked.map(v=>v.name)).size !== allTracked.length) throw new Error('duplicate var declaration')`.
- [x] [Review][Defer] `cleanEnv` test n'herite que PATH/HOME — fragile sur Windows (manque SystemRoot, APPDATA, ...) et CI corporate (NODE_EXTRA_CA_CERTS) [tests/unit/check-required-env.test.ts:52-60] — deferred (matrix runner mono-OS macOS/Linux pour le projet, OK actuellement)
- [x] [Review][Defer] Helpers `minLength`/`matches` retournent `true | string` — anti-pattern type-faillible — deferred refactor en `{ ok, message }` (low priority, internal helpers)
- [x] [Review][Defer] `vi.mocked(createClient).mockResolvedValue(fakeClient as never)` masque type incorrect [tests/integration/paywall/erreur-supabase-transitoire.test.ts:44] — deferred (typage strict Supabase Database trop verbeux pour le scope test)
- [x] [Review][Defer] Mock chaine `.from().select().eq().in().maybeSingle()` replique signature interne SUT — couplage trompeur — deferred (limitation Vitest mocks, candidat refactor wrapper testable)
- [x] [Review][Defer] `EMAIL_DISPLAY` regex accepte tab/control chars/quotes non echappees dans display name [scripts/check-required-env.mjs:28] — deferred (validation shallow, Resend SDK fait la vraie validation)
- [x] [Review][Defer] Asymetrie preview "absent OK / present invalide refuse" cree faux signal "preview vert != prod-ready" [scripts/check-required-env.mjs:163-178] — deferred documentation DECISIONS.md (post-promote rebuild requis)
- [x] [Review][Defer] `OPTOUT_TOKEN_SECRET` REQUIRED tout court sans `requiredOnPreview: false` — friction preview branches sans la var [scripts/check-required-env.mjs:74-75] — deferred verification si emails de rappel actifs en preview
- [x] [Review][Defer] `EMAIL_BASIC` accepte TLD 1 char (`a@b.c`) [scripts/check-required-env.mjs:26] — deferred (typo detection bonus, hors scope strict)
- [x] [Review][Dismiss] AC7/AC8 format message `ERROR (env):` au lieu de `ERROR:` — variation cosmetique plus informative, tests passent
- [x] [Review][Dismiss] Test (i) couvre "presence partielle invalide" pas "toutes absentes" — esprit AC respecte (exit 0 dev local malgre valeurs cassees prouve absence de check shape)
- [x] [Review][Dismiss] AC18 test place en `tests/unit/` au lieu de `tests/integration/` — deviation assumee + justifiee (subprocess pur Node, pas Docker), spec elle-meme hesite ligne 196
- [x] [Review][Dismiss] `NEXT_STEPS.md` mention "Promu 7.A.2" — tracking BMad interne dans doc projet, acceptable
