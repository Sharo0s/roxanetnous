...
- **Procedure droit a l oubli visiteur anonyme** (`notifications_log` avec `user_id IS NULL`, `notifications_ouverture` row avec email + ip mais sans `user_id` car la table n a pas cette colonne) : aucune cascade possible, l utilisateur ne possede pas de compte. Procedure manuelle documentee :
  ...
  2. Traitement manuel par Sylvain via Supabase Studio : `DELETE FROM notifications_log WHERE lower(email) = lower($1)` puis `DELETE FROM notifications_ouverture WHERE lower(email) = lower($1)`.
  3. Trace dans `admin_actions_log` (`action_type = 'rgpd_oubli_visiteur_anonyme'`, `target_id = NULL`, `target_id_text = email_anonymise`) -- conforme au CHECK XOR `target_id_xor` valide depuis story 7.A.7.
...
- **Heritage F5 (idempotence BDD)** : les crons 7.B.2 (purge `notifications_log` 18 mois) et 7.B.3 (anonymisation IP) seront naturellement idempotents par construction. Pattern attendu :
  - `DELETE FROM notifications_log WHERE sent_at < now() - interval '18 months'` : si rien a purger -> 0 row deleted, no-op, pas d erreur.
